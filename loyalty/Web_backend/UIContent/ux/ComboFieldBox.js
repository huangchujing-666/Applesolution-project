...
//        if(!st.isLoading()) {st.load()}   ?‚æ—¶æ³¨é??§ä»¶?ªåŠ¨loading,? ä¸ºå¦‚æ?storeä¸ºç©º?„æ—¶??è¿™é?ä¼šæ˜¯ä¸€ä¸ªæ­»å¾ªçŽ¯
        st.on('load', me.afterSetValue, me, {single: true});
    },    
    getRawValue: function () {        
    	 return Ext.value((this.rawValue=='Please Select'||this.rawValue==undefined?'':this.rawValue), '');//å¤„ç?ie?®é?
    },
    doRawQuery: function() {
     	var me = this,
			qe;
    ...
    onFocus: function() {
    	var me = this,
    		view = me.view;
    	//me.callParent(arguments); //need comment, otherwise ie9 multi drop down list will have problem
