...
namespace Palmary.Loyalty.Web_backend.Controllers
{
    ...
    public class ProductCategoryController : Controller
    {
        ...
        private ProductCategoryLangManager _productCategoryLangManager;
        ...
        public ProductCategoryController()
        {
            ...
            _productCategoryLangManager = new ProductCategoryLangManager();
        }
        ...
        protected override void Initialize(RequestContext requestContext)
        {
            ...
            {
                _productCategory_id = int.Parse(id.ToString());
            }
        ...
        {
            if (_productCategory_id == 0 && !String.IsNullOrEmpty(this.HttpContext.Request["id"]) && this.HttpContext.Request["id"] != "src") // also receive HTTP GET
            {
                _productCategory_id = int.Parse(this.HttpContext.Request["id"]);
            }

            var sql_result = false;
            var productCategory = _productCategoryManager.GetProductCategoryDetailBy(_productCategory_id, ref sql_result);
            var productCategory_lang = _productCategoryLangManager.GetProductCategoryLang_ownedList(_productCategory_id, ref sql_result);

            var formTableJSON = TableFormHandler.GetFormByModule(productCategory, productCategory_lang);
        ...
        public string Insert()
        {
            ...
            var product_cat = new sp_GetProductCategoryDetailByResult();
            product_cat.parent_id = selected_parent_id;

            var formTableJSON = TableFormHandler.GetFormByModule(product_cat, new List<sp_GetProductCategoryLang_ownedListResult>());
        ...
        {         
            var parent_id = collection.GetFormValue(PayloadKeys.ProductCategory.parent_id);
            var leaf = collection.GetFormValue(PayloadKeys.ProductCategory.leaf);
            var category_id = collection.GetFormValue(PayloadKeys.ProductCategory.category_id);
            var display_order = collection.GetFormValue(PayloadKeys.ProductCategory.display_order);
            var status = collection.GetFormValue(PayloadKeys.ProductCategory.status);
            ...
            var new_productCategory_id = 0;
            

            var sql_remark = "";
            if (category_id == 0)
            {
                var addFlag = _productCategoryManager.Create(
                  ...
                  ref new_productCategory_id,
                  ref sql_remark);

                // Create Lang TC
                _productCategoryLangManager.Create(
                                            new_productCategory_id,
            ...
            {
                var updateFlag = _productCategoryManager.Update(
                ...
                // Update Lang TC
                _productCategoryLangManager.Update(
                                            category_id,
                                            (int)Common.CommonConstant.LangCode.tc,
                                            name_tc,
                                            description_tc,
        ...
        public string Tree()
        {
            var resultList = _productCategoryManager.GetProductCategoryLists().ToList();
        ...
        public void TreeMoveUpdate(FormCollection collection)
        {
            ...
            var sql_result = false;
            var sql_remark = "";
            var theCat = _productCategoryManager.GetProductCategoryDetailBy(cat_id, ref sql_result);

            sql_result = _productCategoryManager.Update(
